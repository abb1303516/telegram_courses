...
**Адрес:** http://188.208.103.65:8181

**Важно:** нужна включённая AmneziaVPN (конфиг получить у администратора). Без VPN сервер недоступен.

1. Включите AmneziaVPN
2. Откройте адрес в браузере
3. Введите пароль (спросите у администратора, браузер его запомнит)
3. Вы увидите список файлов курса с превью и датами
4. Файлы с пометкой **«в Telegram»** — ещё не на сервере. Нажмите **«Скачать из TG»** чтобы загрузить на сервер
5. Файлы на сервере можно **скачать на компьютер** или **воспроизвести** прямо в браузере
6. Для массовых действий — отмечайте файлы чекбоксами, появится панель с кнопками
...
- Обход блокировки Telegram через обфусцированный AmneziaWG-туннель (не режется DPI)
- Поддержка прокси (HTTP/SOCKS5) как резервный вариант
