...
**Адрес:** http://173.249.194.240:8181 (с 17.09.2026; прежний адрес http://188.208.103.65:8181 не работает). Если адрес сменится, актуальный подскажет администратор.

Если страница не открывается, включите AmneziaVPN (конфиг у администратора) и попробуйте снова.

1. Откройте адрес в браузере
2. Введите пароль (спросите у администратора, браузер его запомнит)
3. Вы увидите список файлов курса с превью и датами
4. Файлы с пометкой **«в Telegram»** ещё не на сервере. Нажмите **«Скачать из TG»**, чтобы загрузить файл на сервер. Файл появится в списке как скачанный только после полной загрузки
5. Файлы на сервере можно **скачать на компьютер** или **воспроизвести** прямо в браузере
6. Для массовых действий отмечайте файлы чекбоксами, появится панель с кнопками. **«Скачать из TG»** на панели скачивает все отмеченные файлы по очереди, с общим прогрессом («Файл 2 из 5») и кнопкой отмены
7. Список можно сортировать по дате, имени, размеру или типу (кнопки над списком)
8. После скачивания на свой компьютер - **удалите файл с сервера** (место ограничено!)
...
- Сохраняет текст сообщений и подписи к файлам (названия лекций) с кнопкой «Копировать» - даже если копирование в канале запрещено
...
- Скачивает только то, чего ещё нет на сервере; уже скачанные файлы пропускает
- Крупные файлы качаются из Telegram в 4 соединения одновременно: примерно вдвое быстрее на NL и вчетверо быстрее из США, чем одним потоком
- Недокачанный файл не показывается как скачанный (загрузка идёт во временный файл `.part`)
...
- Работает на зарубежном сервере с прямым доступом к Telegram; на случай блокировок есть поддержка прокси (HTTP/SOCKS5)
- Полоса прогресса при скачивании из Telegram (байты в реальном времени) с кнопкой отмены
- Сортировка по дате поста, имени, размеру, типу (по умолчанию - по дате, новые сверху; выбор запоминается)
- Дата публикации отображается у каждого файла
...
HOST=0.0.0.0               # принимать подключения снаружи
PORT=8181
```

Необязательно (параллельная загрузка крупных файлов):
```
DOWNLOAD_CONNECTIONS=4     # 1 = качать одним потоком
PARALLEL_MIN_MB=60         # файлы меньше качаются одним потоком
```
...
Откройте в браузере: `http://ваш-ip:8181` (порт из `PORT`)
...
- Ограничьте доступ через **firewall** (ufw):
  ```bash
  sudo ufw allow from ваш_домашний_ip to any port 8181
  ```
...
| "Уже идёт загрузка" | Дождитесь завершения текущей загрузки или нажмите «Отменить загрузку» |
| Файл долго качается на компьютер | Скорость до компьютера зависит от сервера, сообщите администратору |
...
| Порт не открывается | `sudo ufw allow 8181/tcp` или проверьте настройки VDS |
...
├── parallel_download.py # Параллельная загрузка крупных файлов
├── config.py           # Конфигурация
├── .env                # Ваши настройки (не коммитить!)
├── .env.example        # Шаблон настроек
├── requirements.txt    # Зависимости Python
├── docs/               # Бриф на параллельную загрузку (история)
